...
void sweep() {
  ...
  while(chunk < mem_heap_hi()&& chunk_size(chunk)){
    void* tempChunk = next_chunk(chunk);
...
void * is_pointer(void * ptr) {
  ...
  if(ptr <mem_heap_lo()|| ptr>mem_heap_hi()){
    ...
    void* pointer = mem_heap_lo();
    
    while(chunk_size(pointer)){
    if(in_use(pointer)&& ptr < pointer + chunk_size(pointer)&& pointer <= ptr){
      return pointer;
    }
    pointer = next_chunk(pointer);
    }
  }
